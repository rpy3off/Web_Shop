    ...
    >
      ...
      <Stack spacing='6'>
        <Flex justify='space-between'>
          <Text fontWeight='medium' color={mode('gray.500', 'gray.100')}>
            Subtotal
          </Text>
        ...
        <Flex justify='space-between'>
          <Text fontSize='xl' fontWeight='extrabold' color={mode('gray.500', 'gray.100')}>
            Total
          </Text>
          <Text fontWeight='medium'>${Number(subtotal) + Number(shipping)}</Text>
        </Flex>
      </Stack>
      <Button as={ReactLink} to='/checkout' colorScheme='cyan' size='lg' fontSize='medium' rightIcon={<FaArrowRight />}>
        Checkout
      </Button>
    </Stack>
  );
